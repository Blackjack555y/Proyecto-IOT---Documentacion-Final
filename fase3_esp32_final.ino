// SmartJoy — Fase 3 Final: ESP32 + MQTT TLS con certificado ISRG Root X1
// Formato PROGMEM validado por el equipo de HiveMQ (febrero 2025)
...
// ── Certificado ISRG Root X1 — formato PROGMEM validado por HiveMQ ──
...
void setup() {
  ...
  Serial.println("SmartJoy listo — publicando por MQTT con TLS");
}
...
void loop() {
  ...
  if (now - lastMsg > 50) {
    ...
    char msgMouse[64];
    snprintf(msgMouse, sizeof(msgMouse), "{\"vx\":%d,\"vy\":%d}", vel_x, vel_y);
    mqtt.publish(TOPIC_MOUSE, msgMouse);

    char msgKeys[64];
    snprintf(msgKeys, sizeof(msgKeys), "{\"key\":\"%s\",\"btn\":%d}", tecla, btn ? 1 : 0);
